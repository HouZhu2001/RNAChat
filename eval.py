...
def get_simcse(simcse_path, func_text):
    ...
    # Filter out any remaining empty predictions
    valid_func_text = []
    for item in func_text:
        if item['predict_func'] and item['predict_func'].strip() and item['predict_func'].strip() != "###Assistant:":
            valid_func_text.append(item)
        else:
            print(f"⚠️  Skipping empty prediction for item: {item.get('name', 'unknown')}")
    
    if not valid_func_text:
        print("❌ ERROR: No valid predictions found after filtering!")
        return {"error": "No valid predictions"}
    
    print(f"Calculating metrics for {len(valid_func_text)} valid predictions...")
    
    refs = [item['correct_func'] for item in valid_func_text]
    cands = [item['predict_func'] for item in valid_func_text]
    ...
    for i in range(len(valid_func_text)):
        item = valid_func_text[i]
        correct = item['correct_func'].split()
        predict = item['predict_func'].split()
        
        # Additional safety check for empty predictions
        if not predict:
            print(f"⚠️  Skipping item with empty prediction tokens: {item.get('name', 'unknown')}")
            continue
            
        bleu = sentence_bleu([correct], predict, weights)
        simcse = 1 - cosine(ref_embeddings[i], cand_embeddings[i])
        valid_func_text[i]['simcse'] = simcse
        valid_func_text[i]['bleu'] = bleu
    ...
    for ngram in range(4):
        if bleu_list[ngram]:  # Only calculate if we have valid scores
            scores[f'average_bleu_{str(ngram+1)}'] = sum(bleu_list[ngram]) / len(bleu_list[ngram])
        else:
            scores[f'average_bleu_{str(ngram+1)}'] = 0.0
        
    if simcse_list:  # Only calculate if we have valid scores
        scores['average_simcse'] = sum(simcse_list) / len(simcse_list)
    else:
        scores['average_simcse'] = 0.0
        
    print("Average scores:")
    print(scores)

    valid_func_text.append(scores)

    return valid_func_text
